...
object SMerger5 {
  ...
  def setCmdWithDir1(dirname:String, partname:String, samtools:String, rmap:mutable.HashMap[String, Array[String]], sambam:String="bam", thread:Int=4):mutable.Iterable[String] = {
    ...
    if(fileLen > 5) {
      while(!exitv) {
        ...
        val cmd = s"$samtools merge -@ ${thread} -O ${sambam} $dirname/$partname$loop-$rname.merged $filex"
  ...
  def main(args: Array[String]): Unit = {
    ...
    dirs.foreach{dir =>
      println(s"Search in $dirPath/$dir")
    ...
    while(i < fsize) {
      ...
      val res = setCmdWithDir1(dirPath, partName, samtools, rnameMap, sambam, thread)
