      ...
      val rdd1 = sc.textFile(l.mkString(",")).filter(!_.startsWith("@")).map{ x =>
        ...
        .map { x =>
          val paired = x.map { sam =>
            ...
            val idxedRname = bcMap.get(sam.rname).getOrElse(0) * 1000000 //idx * 1000
            val dividedPos = (sam.pos / 1000000).toInt //position으로 키를 조금 더 늘림
...
//          println(s"out dir is :: $absolPath/loop${"%03d".format(lcnt)}")
          streamDir.mkdirs()
